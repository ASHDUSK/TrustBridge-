...
import torch
torch.backends.cudnn.benchmark = True  # TrustBridge patch: 固定输入形状，加速卷积
torch.set_float32_matmul_precision("high")  # TrustBridge patch: TF32
...
class BridgeRunner(L.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # Predict x0
        x0_pred = self.diffusion.sample_x0(y, self.generator).float()  # TrustBridge patch: bf16 -> fp32

        loss = F.mse_loss(x0_pred, x0)
        metrics = compute_metrics(x0.float(), x0_pred.float())  # TrustBridge patch: bf16 -> fp32
